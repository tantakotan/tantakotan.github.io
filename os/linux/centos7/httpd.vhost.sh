...
# Edit 20170306
v_github_dir="raw.githubusercontent.com/tantakotan/tantakotan.github.io/master/os/linux"
...

{
	...
	[ "$(diff "${v_httpd_conf}${v_backup_suffix}" "${v_httpd_conf}")" ] || \mv -f "${v_httpd_conf}${v_backup_suffix}" "${v_httpd_conf}"
...
__EOD__
	...
	[ "$(diff "${v_httpd_vhost_fqdn_conf}${v_backup_suffix}" "${v_httpd_vhost_fqdn_conf}")" ] || \mv -f "${v_httpd_vhost_fqdn_conf}${v_backup_suffix}" "${v_httpd_vhost_fqdn_conf}"
